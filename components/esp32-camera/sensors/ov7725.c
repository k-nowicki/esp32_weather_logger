...
static int set_reg_bits(cam_sensor_t *sensor, uint8_t reg, uint8_t offset, uint8_t length, uint8_t value)
{
    int ret = 0;
...
static int get_reg_bits(cam_sensor_t *sensor, uint8_t reg, uint8_t offset, uint8_t length)
{
    int ret = 0;
    ret = SCCB_Read(sensor->slv_addr, reg);
    if(ret < 0){
...
static int set_dummy(cam_sensor_t *sensor, int val){ return -1; }
static int set_gainceiling_dummy(cam_sensor_t *sensor, gainceiling_t val){ return -1; }
static int set_res_raw(cam_sensor_t *sensor, int startX, int startY, int endX, int endY, int offsetX, int offsetY, int totalX, int totalY, int outputX, int outputY, bool scale, bool binning){return -1;}
static int _set_pll(cam_sensor_t *sensor, int bypass, int multiplier, int sys_div, int root_2x, int pre_div, int seld5, int pclk_manual, int pclk_div){return -1;}
...
int ov7725_init(cam_sensor_t *sensor)
{
