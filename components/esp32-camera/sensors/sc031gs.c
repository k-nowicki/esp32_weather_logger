...
static int set_reg_bits(cam_sensor_t *sensor, uint16_t reg, uint8_t offset, uint8_t length, uint8_t value)
{
    int ret = 0;
...
static int set_special_effect(cam_sensor_t *sensor, int sleep_mode_enable) // For sc03ags sensor, This API used for sensor sleep mode control.
{
...
int set_bpc(cam_sensor_t *sensor, int enable) // // For sc03ags sensor, This API used to control BLC
{
    int ret = 0;
...
static int set_output_window(cam_sensor_t *sensor, int offset_x, int offset_y, int w, int h)
{
    int ret = 0;
...
int sc031gs_init(cam_sensor_t *sensor)
{
