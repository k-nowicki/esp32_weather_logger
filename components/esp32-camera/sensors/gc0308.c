...
static int set_contrast(cam_sensor_t *sensor, int contrast)
{
    if (contrast != 0) {
        write_reg(sensor->slv_addr, 0xfe, 0x00);
...
static int set_global_gain(cam_sensor_t *sensor, int gain_level)
{
    if (gain_level != 0) {
        write_reg(sensor->slv_addr, 0xfe, 0x00);
...
int gc0308_init(cam_sensor_t *sensor)
{
