...
static int set_bank(cam_sensor_t *sensor, ov2640_bank_t bank)
{
    int res = 0;
    if (bank != reg_bank) {
        reg_bank = bank;
...
static int write_regs(cam_sensor_t *sensor, const uint8_t (*regs)[2])
{
    int i=0, res = 0;
    while (regs[i][0]) {
...
static int write_reg(cam_sensor_t *sensor, ov2640_bank_t bank, uint8_t reg, uint8_t value)
{
...
static int set_reg_bits(cam_sensor_t *sensor, uint8_t bank, uint8_t reg, uint8_t offset, uint8_t mask, uint8_t value)
{
    int ret = 0;
...
static int read_reg(cam_sensor_t *sensor, ov2640_bank_t bank, uint8_t reg)
{
...
static uint8_t get_reg_bits(cam_sensor_t *sensor, uint8_t bank, uint8_t reg, uint8_t offset, uint8_t mask)
{
    return (read_reg(sensor, bank, reg) >> offset) & mask;
}

static int write_reg_bits(cam_sensor_t *sensor, uint8_t bank, uint8_t reg, uint8_t mask, int enable)
{
...
static int reset(cam_sensor_t *sensor)
{
    int ret = 0;
...
static int set_pixformat(cam_sensor_t *sensor, pixformat_t pixformat)
{
    int ret = 0;
...
static int set_window(cam_sensor_t *sensor, ov2640_sensor_mode_t mode, int offset_x, int offset_y, int max_x, int max_y, int w, int h){
    int ret = 0;
...
static int set_framesize(cam_sensor_t *sensor, framesize_t framesize)
{
    int ret = 0;
...
static int set_contrast(cam_sensor_t *sensor, int level)
{
    int ret=0;
    level += 3;
...
static int set_brightness(cam_sensor_t *sensor, int level)
{
    int ret=0;
    level += 3;
...
static int set_saturation(cam_sensor_t *sensor, int level)
{
    int ret=0;
    level += 3;
...
static int set_special_effect(cam_sensor_t *sensor, int effect)
{
    int ret=0;
    effect++;
...
static int set_wb_mode(cam_sensor_t *sensor, int mode)
{
    int ret=0;
...
static int set_ae_level(cam_sensor_t *sensor, int level)
{
    int ret=0;
    level += 3;
...
static int set_quality(cam_sensor_t *sensor, int quality)
{
    if(quality < 0) {
        quality = 0;
    } else if(quality > 63) {
...
static int set_agc_gain(cam_sensor_t *sensor, int gain)
{
    if(gain < 0) {
        gain = 0;
    } else if(gain > 30) {
...
static int set_gainceiling_sensor(cam_sensor_t *sensor, gainceiling_t gainceiling)
{
...
static int set_aec_value(cam_sensor_t *sensor, int value)
{
    if(value < 0) {
        value = 0;
    } else if(value > 1200) {
...
//unsupported
static int set_sharpness(cam_sensor_t *sensor, int level)
{
   return -1;
}

static int set_denoise(cam_sensor_t *sensor, int level)
{
   return -1;
}

static int get_reg(cam_sensor_t *sensor, int reg, int mask)
{
...
static int set_reg(cam_sensor_t *sensor, int reg, int mask, int value)
{
    int ret = 0;
...
static int set_res_raw(cam_sensor_t *sensor, int startX, int startY, int endX, int endY, int offsetX, int offsetY, int totalX, int totalY, int outputX, int outputY, bool scale, bool binning)
{
    return set_window(sensor, (ov2640_sensor_mode_t)startX, offsetX, offsetY, totalX, totalY, outputX, outputY);
}

static int _set_pll(cam_sensor_t *sensor, int bypass, int multiplier, int sys_div, int root_2x, int pre_div, int seld5, int pclk_manual, int pclk_div)
{
    return -1;
}

static int set_xclk(cam_sensor_t *sensor, int timer, int xclk)
{
    int ret = 0;
...
static int init_status(cam_sensor_t *sensor){
...
int ov2640_init(cam_sensor_t *sensor)
{
    sensor->reset = reset;
